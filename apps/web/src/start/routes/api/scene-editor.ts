...
async function post(request: Request) {
  ...
  if (!request.headers.get('content-type')?.toLowerCase().startsWith('application/json')) {
    return Response.json({ error: 'Scene editor saves require JSON.' }, { status: 415 })
  }
  ...
  if (
    ...
    return Response.json({ error: 'Invalid HQ scene placement payload.' }, { status: 400 })
  }
  ...
  if (!sourcePath.startsWith(`${repoRoot}${path.sep}`)) {
    return Response.json({ error: 'Invalid scene path.' }, { status: 400 })
  }
...
export const Route = createFileRoute('/api/scene-editor')({
  server: {
    handlers: {
