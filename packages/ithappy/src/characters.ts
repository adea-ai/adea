// ithappy character system for HQ.
//
...
const characterAnimationMaps: Record<IthappyCharacterId, Record<string, string>> = {
...
const characterFiles: Record<IthappyCharacterId, string> = {
...
// ithappy characters are authored at real-world scale (~1.73m tall in GLB
...
function getManifest(id: string): CharacterManifest | undefined {
  if (isIthappyCharacterId(id)) {
    return {
      id,
      label: ithappyCharacterLabels[id],
      assetUrl: `${assetRoot}/${characterFiles[id]}`,
      // Animations are embedded in the GLB; no separate animation files.
      bodyAnimationUrls: {},
      // No Unity UV-offset facial textures or expression clips.
      facialTextureUrls: { eye: "", mouth: "" },
      facialAnimationUrls: {},
    };
  }
  if (isIthappyCustomCharacterId(id)) {
    const preset = ithappyCustomCharacterPresets[id];
    return {
      id,
      label: preset.label,
...
async function loadIthappyCharacter(loader: GLTFLoader, id: string): Promise<LoadedCharacter> {
  // Custom assembled character from Creative Character parts.
  if (isIthappyCustomCharacterId(id)) {
    const preset = ithappyCustomCharacterPresets[id];
    const assembled = await assembleIthappyCharacter(loader, preset.config);
  ...
  if (!manifest) throw new Error(`Unknown ithappy character: ${id}`);
...
async function ensureEmbeddedClips(
  ...
  // Don't call assembleIthappyCharacter here — that would conflict with
...
async function loadIthappyCharacterAnimations(
  loader: GLTFLoader,
  id: string,
  keys: readonly string[],
): Promise<LoadedCharacterAnimations> {
  // Custom characters use the Adult-rig animation map (same skeleton).
  const animMap = isIthappyCustomCharacterId(id)
    ? ADULT_ANIMATION_MAP
    : isIthappyCharacterId(id)
...
const ithappyProvider: CharacterProvider = {
  characterIds: allIthappyCharacterIds,
  getManifest,
  loadCharacter: loadIthappyCharacter,
  loadAnimatedCharacter: loadIthappyCharacter,
  loadCharacterAnimations: loadIthappyCharacterAnimations,
};

// Register on module import so any scene that imports @agent-hq/ithappy/characters
// automatically plugs into the shared character pipeline.
registerCharacterProvider(ithappyProvider);
...
export const ithappyCharacterPartIds = [
  "body-010",
  "clown-nose-001",
  "costume-10-001",
  "costume-6-001",
...
export type IthappyCharacterPartId = (typeof ithappyCharacterPartIds)[number];

const characterPartFiles: Record<IthappyCharacterPartId, string> = {
...
const characterPartRoot = "/assets/ithappy/character-parts";

export const ithappyCharacterPartAssets = ithappyCharacterPartIds.map((id) => ({
  id,
