...
export function RoomDesigner({
  ...
  useEffect(() => {
    ...
    const attach = async () => {
      ...
      try {
        ...
        setStatus(loadedPlacements.length ? 'Loaded saved room props.' : 'Drag a prop onto a room.')
      } catch {
  ...
  useEffect(() => {
    if (enabled) return
    if (JSON.stringify(placementsRef.current) === JSON.stringify(savedPlacementsRef.current)) return
